...
from file_generator import generate_file_version_info, generate_nsis_file
...
if os_name == "windows" and os.path.isfile(lrelease_exe):
    folders = [source_dir] + [
        ...
        for dI in os.listdir(components_dir)
        if os.path.isdir(os.path.join(components_dir, dI))
    ]
    files_to_process = []
...
generate_file_version_info(
    file_version_info_path,
    name=APP_NAME,
    owner=APP_OWNER,
...
pyinstaller_args += [
    ...
    f"--icon={APP_ICON}",
    os.path.join(source_dir, "src", "edobot", "main.py"),
]
...
shutil.copytree(
    ...
    dirs_exist_ok=True,
    copy_function=copy_function,
    ignore=lambda _, c: (f for f in c if f.endswith(".qm")),
)
